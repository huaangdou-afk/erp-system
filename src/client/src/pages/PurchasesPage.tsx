...
import { usePurchaseStore, PurchaseOrder } from '../stores/purchaseStore';
...
export default function PurchasesPage() {
  const { orders, loading, fetch, create, updateStatus, remove } = usePurchaseStore();
  ...
  const updateLine = (i: number, field: keyof LineItem, value: number | string) => {
    ...
    if (field === 'product_id') {
      ...
      if (p) {
        updated[i].unit_price = p.purchase_price;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!supplierId || lines.some((l) => !l.product_id)) return;
    setSaving(true);
    try {
      await create({
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-xl font-bold text-slate-800">采购管理</h1>
        <button className="btn btn-primary" onClick={openCreate}>+ 新建采购单</button>
      </div>

      <DataTable columns={columns} data={orders} loading={loading} searchPlaceholder="搜索单号..." renderRowActions={renderActions} />

      {/* Create Modal */}
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="新建采购单" size="lg">
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div>
            <label className="label">供应商 *</label>
            <select className="input" value={supplierId} onChange={e => setSupplierId(Number(e.target.value))} required>
          ...
          <div className="flex justify-end gap-2">
            ...
            <button type="submit" className="btn btn-primary" disabled={saving}>{saving ? '保存中...' : '确认采购'}</button>
          </div>
        </form>
      </Modal>

      {/* Detail Modal */}
      <Modal open={detailId !== null} onClose={() => setDetailId(null)} title="采购单明细" size="lg">
        {selectedOrder && (
          <div className="flex flex-col gap-3">
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div><span className="text-slate-500">单号：</span>{selectedOrder.order_no}</div>
              <div><span className="text-slate-500">供应商：</span>{selectedOrder.supplier_name}</div>
