...
// POST /api/suppliers
router.post('/', createSupplierRules, validate, async (req, res, next) => {
  try {
    const db = await getDb();
...
router.put('/:id', updateSupplierRules, validate, async (req, res, next) => {
  try {
    const db = await getDb();
    ...
    if (!check.step()) { check.free(); return res.status(404).json({ success: false, error: '供应商不存在' }); }
    check.free();
...
router.delete('/:id', async (req, res, next) => {
  try {
    ...
    ref.bind([id]);
    if (ref.step()) { ref.free(); return res.status(400).json({ success: false, error: '该供应商有采购单关联，无法删除' }); }
    ref.free();
